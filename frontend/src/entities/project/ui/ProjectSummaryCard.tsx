 ...
 *
 * Pure display component for project section summary.
...
export interface ProjectSummaryCardProps {
  ...
  onClick?: (section: ProjectSection) => void;
}
 ...
 * - Renders section summary data in card format
 * - Delegates click action via callback (parent handles navigation)
 *
 * @example
 * ```tsx
 * function ProjectViewPage() {
 *   const { data: summary } = useQuery(projectQueries.summary(projectId));
 ...
 *           key={section.section}
 *           summary={section}
 *           onClick={(s) => navigate(`/projects/${projectId}/${s}`)}
 *         />
 *       ))}
 *     </div>
 *   );
