 ...
 * Quotation Info Card
 *
 * Presentational component for displaying quotation details and line items.
 * Used by both QuotationDetailsPanel (inline tab) and QuotationDetailPage (full page).
 *
 * Features:
...
export interface QuotationInfoCardProps {
  /** Quotation data to display */
...
/**
 * Quotation details and line items display component.
 */
export function QuotationInfoCard({
  ...
  className,
}: Readonly<QuotationInfoCardProps>) {
  return (
        ...
        <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
          ...
          <div>
            ...
            <p className="text-steel-300">
              {formatQuotationDate(quotation.createdAt, includeTimeInDates)}
            </p>
          </div>
          <div>
            <span className="text-steel-500">Quotation Date</span>
            <p className="text-steel-300">
              {formatQuotationDate(quotation.quotationDate, includeTimeInDates)}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-steel-300">
              {formatQuotationDate(quotation.expiryDate, includeTimeInDates)}
            </p>
          </div>
        </div>
              ...
              quotation.lineItems.map((item, index) => (
                <Table.Row key={item.id}>
                  ...
                  <Table.Cell className="text-right font-medium text-copper-400">
                    {formatQuotationCurrency(item.lineTotal)}
                  </Table.Cell>
                </Table.Row>
              ))
            ) : (
        ...
        <div className="border-t border-steel-700/50 bg-steel-800/30 px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xl font-bold text-copper-400">
              {formatQuotationCurrency(quotation.totalAmount)}
            </span>
          </div>
        </div>
      </Card>
