...
The `lib/` subdirectory contains internal hooks for UI components:
...
For className merging, use `cn` from `@/shared/lib/cn`.
...
For the className utility, import directly from `shared/lib`:

```typescript
import { cn } from '@/shared/lib/cn';
```

Note: `useFocusTrap` and `useBodyScrollLock` hooks are internal to `shared/ui` and should not be imported from outside.
...
The `lib/` subdirectory contains internal hooks used by UI components (Modal, Combobox, etc.):
...
These hooks are **internal implementation details** and should not be imported from outside `shared/ui/`. If you need similar functionality for custom components, consider using the provided Modal component or creating a new shared hook in `shared/lib/`.

## Utilities

### `cn` - ClassName Utility

Located in `@/shared/lib/cn`. Utility function for conditional className merging:
...
# Specific component
npm test src/shared/ui/forms/Combobox
```
