...
describe('useUpdateCompany', () => {
  ...
  describe('mutation execution', () => {
    it('should call updateCompany command on mutate', async () => {
    ...
    it('should return command result on success', async () => {
      const commandResult = createCommandResult(1, 'Company updated');
  ...
  describe('cache invalidation', () => {
    it('should invalidate list and detail queries on success', async () => {
      ...
      // Also invalidates specific detail query
      expect(invalidateSpy).toHaveBeenCalledWith(
        expect.objectContaining({ queryKey: ['companies', 'detail', 1] })
      );
    });
  });
  ...
  describe('callbacks', () => {
    it('should call onSuccess callback with result', async () => {
      ...
      const { result } = renderHook(() => useUpdateCompany({ onSuccess }), {
        wrapper: createQueryWrapper(),
      });
    ...
    it('should call onError callback with error', async () => {
      ...
      const { result } = renderHook(() => useUpdateCompany({ onError }), {
        wrapper: createQueryWrapper(),
      });
