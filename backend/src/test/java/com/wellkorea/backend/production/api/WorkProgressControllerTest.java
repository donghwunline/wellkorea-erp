...
class WorkProgressControllerTest extends BaseIntegrationTest implements TestFixtures {
    ...
    private void insertTestProject() {
        // Use project ID 100 to avoid conflicts with seed data (projects 1-3)
        jdbcTemplate.update(
                "INSERT INTO projects (id, job_code, customer_company_id, project_name, due_date, " +
                        ...
                        "VALUES (100, 'WK2K25-0100-0105', 1, 'Test Project', '2025-02-28', " +
    ...
    private void insertTestWorkProgressSheet() {
        // Insert a work progress sheet with ID 100 for test project 100 to avoid seed data conflicts
        jdbcTemplate.update(
                "INSERT INTO work_progress_sheets (id, project_id, product_id, quantity, sequence, status) " +
                        "VALUES (100, 100, 1, 10, 1, 'NOT_STARTED') " +
        ...
        jdbcTemplate.update(
                "INSERT INTO work_progress_steps (id, sheet_id, step_template_id, step_number, step_name, status, estimated_hours) " +
                        "SELECT nextval('work_progress_steps_id_seq'), 100, t.id, t.step_number, t.step_name, 'NOT_STARTED', t.estimated_hours " +
    ...
    class GetWorkProgressSheetsTests {
        ...
        @Test
        @DisplayName("should return 200 with paginated list when Production lists work progress sheets")
        void getWorkProgressSheets_AsProduction_Returns200() throws Exception {
            ...
            mockMvc.perform(get(WORK_PROGRESS_URL)
                    ...
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.success").value(true))
                    // Page<T> response: data is wrapped in content array
                    .andExpect(jsonPath("$.data.content", hasSize(greaterThanOrEqualTo(1))))
                    .andExpect(jsonPath("$.data.content[0].id").isNumber())
                    .andExpect(jsonPath("$.data.content[0].projectId").value(100))
                    .andExpect(jsonPath("$.data.content[0].productId").value(1))
                    .andExpect(jsonPath("$.data.content[0].status").value("NOT_STARTED"))
                    // Pagination metadata
        ...
        void getWorkProgressSheets_WithProgress_Returns200() throws Exception {
            ...
            mockMvc.perform(get(WORK_PROGRESS_URL)
                            ...
                            .param("projectId", "100"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.data.content[0].progressPercentage").exists());
        }
    ...
    class GetWorkProgressSheetDetailTests {
        ...
        void getWorkProgressSheetDetail_AsProduction_Returns200() throws Exception {
            ...
            mockMvc.perform(get(WORK_PROGRESS_URL + "/100")
                            .header("Authorization", "Bearer " + productionToken))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.success").value(true))
    ...
    class UpdateStepStatusTests {
        ...
        void updateStepStatus_ToInProgress_Returns200() throws Exception {
            ...
            Long stepId = jdbcTemplate.queryForObject(
                    "SELECT id FROM work_progress_steps WHERE sheet_id = 100 ORDER BY step_number LIMIT 1",
                    Long.class
            );
            ...
            mockMvc.perform(put(WORK_PROGRESS_URL + "/100/steps/" + stepId)
                            .header("Authorization", "Bearer " + productionToken)
                            .contentType(MediaType.APPLICATION_JSON)
        ...
        void updateStepStatus_Outsourced_Returns200() throws Exception {
            ...
            jdbcTemplate.update(
                    "INSERT INTO companies (id, name, is_active) VALUES (200, 'Outsource Vendor', true) " +
                            "ON CONFLICT (id) DO NOTHING"
            );
            jdbcTemplate.update(
                    "INSERT INTO company_roles (company_id, role_type) VALUES (200, 'OUTSOURCE') " +
        ...
        void updateStepStatus_NonExistentStep_Returns404() throws Exception {
            ...
            mockMvc.perform(put(WORK_PROGRESS_URL + "/100/steps/999")
                            .header("Authorization", "Bearer " + productionToken)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(updateRequest))
                    .andExpect(status().isNotFound());
    ...
    class GetProjectProductionSummaryTests {
        ...
        void getProjectSummary_Returns200() throws Exception {
            ...
            mockMvc.perform(get(WORK_PROGRESS_URL + "/project/100/summary")
                            .header("Authorization", "Bearer " + productionToken))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success").value(true))
                    .andExpect(jsonPath("$.data.projectId").value(100))
    ...
    // ==========================================================================
    // GET /api/work-progress/project/{projectId}/outsourced - Outsourced Steps
    // ==========================================================================

    @Nested
    @DisplayName("GET /api/work-progress/project/{projectId}/outsourced - Outsourced Steps")
    class GetOutsourcedStepsTests {

        @Test
        @DisplayName("should return 200 with empty list when no outsourced steps")
        void getOutsourcedSteps_NoOutsourced_Returns200() throws Exception {
            insertTestWorkProgressSheet();

            mockMvc.perform(get(WORK_PROGRESS_URL + "/project/100/outsourced")
                            .header("Authorization", "Bearer " + productionToken))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success").value(true))
        ...
        @Test
        @DisplayName("should return 200 with outsourced steps when they exist")
        void getOutsourcedSteps_WithOutsourced_Returns200() throws Exception {
            insertTestWorkProgressSheet();
            ...
            // Mark a step as outsourced
            jdbcTemplate.update(
                    "UPDATE work_progress_steps SET is_outsourced = true, outsource_vendor_id = 200, " +
                            "outsource_eta = CURRENT_DATE + INTERVAL '5 days', outsource_cost = 50000 " +
                            "WHERE sheet_id = 100 AND step_number = 2"
            );
            ...
            mockMvc.perform(get(WORK_PROGRESS_URL + "/project/100/outsourced")
                    ...
                    .andExpect(jsonPath("$.data[0].isOutsourced").value(true))
                    .andExpect(jsonPath("$.data[0].outsourceVendorId").value(200));
        }

        @Test
        @DisplayName("should return 403 when Sales accesses outsourced steps")
        void getOutsourcedSteps_AsSales_Returns403() throws Exception {
            mockMvc.perform(get(WORK_PROGRESS_URL + "/project/100/outsourced")
                            .header("Authorization", "Bearer " + salesToken))
                    .andExpect(status().isForbidden());
        }
    ...
    // ==========================================================================
    // DELETE /api/work-progress/{id} - Delete Work Progress Sheet
    // ==========================================================================

    @Nested
    @DisplayName("DELETE /api/work-progress/{id} - Delete Work Progress Sheet")
    class DeleteSheetTests {

        @Test
        @DisplayName("should return 200 when Admin deletes sheet")
        void deleteSheet_AsAdmin_Returns200() throws Exception {
            insertTestWorkProgressSheet();

            mockMvc.perform(delete(WORK_PROGRESS_URL + "/100")
                            .header("Authorization", "Bearer " + adminToken))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success").value(true))
                    .andExpect(jsonPath("$.data.id").value(100))
                    .andExpect(jsonPath("$.data.message").value("Work progress sheet deleted successfully"));
        }

        @Test
        @DisplayName("should return 403 when Production deletes sheet")
        void deleteSheet_AsProduction_Returns403() throws Exception {
            insertTestWorkProgressSheet();
        ...
        @Test
        @DisplayName("should return 403 when Finance deletes sheet")
        void deleteSheet_AsFinance_Returns403() throws Exception {
            insertTestWorkProgressSheet();
        ...
        @Test
        @DisplayName("should return 403 when Sales deletes sheet")
        void deleteSheet_AsSales_Returns403() throws Exception {
            mockMvc.perform(delete(WORK_PROGRESS_URL + "/100")
                            .header("Authorization", "Bearer " + salesToken))
                    .andExpect(status().isForbidden());
        }
        ...
        @Test
        @DisplayName("should return 404 when deleting non-existent sheet")
        void deleteSheet_NonExistent_Returns404() throws Exception {
            mockMvc.perform(delete(WORK_PROGRESS_URL + "/999")
                            .header("Authorization", "Bearer " + adminToken))
                    .andExpect(status().isNotFound());
        }
    }
