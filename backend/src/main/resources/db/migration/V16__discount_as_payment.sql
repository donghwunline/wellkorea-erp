...
SELECT i.id, i.issue_date, i.discount_amount, 'DISCOUNT', NULL, 'Migrated from invoice discount field', i.created_by_id
FROM tax_invoices i
WHERE i.discount_amount > 0;

-- 3. Update totalAmount to gross (add back discount that was previously subtracted)
UPDATE tax_invoices SET total_amount = total_amount + discount_amount
WHERE discount_amount > 0;

-- 4. Drop discount_amount column
ALTER TABLE tax_invoices DROP CONSTRAINT chk_invoice_discount_non_negative;
ALTER TABLE tax_invoices DROP COLUMN discount_amount;
