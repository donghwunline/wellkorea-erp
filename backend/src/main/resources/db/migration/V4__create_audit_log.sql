...
COMMENT ON TABLE audit_logs IS 'Immutable audit trail for all data changes and sensitive access (US9 requirement)';
...
COMMENT ON FUNCTION prevent_audit_log_modification() IS 'Enforces immutability of audit log';
COMMENT ON FUNCTION audit_trigger_function() IS 'Generic trigger function to automatically log all changes to sensitive tables';
