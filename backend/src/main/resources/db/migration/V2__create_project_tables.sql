...
CREATE TABLE projects (
    ...
    job_code VARCHAR(20) NOT NULL UNIQUE,  -- WK2{year}-{sequence}-{date} format
    ...
    CONSTRAINT chk_job_code_format CHECK (job_code ~ '^WK2\d{4}-\d{6}-\d{8}$')
);
...
COMMENT ON COLUMN projects.job_code IS 'Unique business identifier: WK2{year}-{sequence}-{date}';
COMMENT ON COLUMN projects.status IS 'Project lifecycle status: DRAFT, ACTIVE, COMPLETED, ARCHIVED';

COMMENT ON TABLE products IS 'Product catalog for quotations, production, and invoicing';
COMMENT ON COLUMN products.sku IS 'Product code (unique when active)';
COMMENT ON COLUMN products.base_unit_price IS 'Optional catalog price - can be overridden per quotation';

COMMENT ON TABLE product_types IS 'Product categories with manufacturing step templates';
COMMENT ON TABLE work_progress_step_templates IS 'Manufacturing step templates per product type (e.g., Design → Laser → Sheet Metal → Assembly)';
