...
/**
 * Service for multi-level approval workflow (결재 라인).
 * Handles approval chain configuration and sequential approval processing.
 */
@Service
@Transactional
public class ApprovalService {
     ...
     */
    public ApprovalRequest approve(Long approvalRequestId, Long approverUserId, String comments) {
     ...
     */
    public ApprovalRequest reject(Long approvalRequestId, Long approverUserId, String reason, String comments) {
    ...
    public Page<ApprovalRequest> listPendingApprovals(Long userId, Pageable pageable) {
        return approvalRequestRepository.findPendingByApproverUserId(userId, pageable);
    }
     ...
     */
    @Transactional(readOnly = true)
    public Page<ApprovalRequest> listAllApprovals(EntityType entityType, ApprovalStatus status, Pageable pageable) {
        return approvalRequestRepository.findAllWithFilters(entityType, status, pageable);
    }

    /**
     * Get approval details with all level decisions.
     */
    @Transactional(readOnly = true)
    public ApprovalRequest getApprovalDetails(Long approvalRequestId) {
        return approvalRequestRepository.findByIdWithLevelDecisions(approvalRequestId)
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Approval request not found with ID: " + approvalRequestId));
    }
     ...
     */
    @Transactional(readOnly = true)
    public List<ApprovalHistory> getApprovalHistory(Long approvalRequestId) {
        // Verify request exists
        approvalRequestRepository.findById(approvalRequestId)
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Approval request not found with ID: " + approvalRequestId));

        return historyRepository.findByApprovalRequestIdOrderByCreatedAtAsc(approvalRequestId);
    }
     ...
     */
    public ApprovalChainTemplate updateChainLevels(Long chainTemplateId, List<ChainLevelCommand> commands) {
